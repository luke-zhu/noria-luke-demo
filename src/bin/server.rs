...
//    let zookeeper_addr = "noria-zookeeper-headless:2181/demo";
    ...
    let records = snapshots.lookup(&[user.to_lowercase().into()], true).unwrap();

    if records.is_empty() {
        return Html(format!("<h1>{}</h1><p>The user has not posted anything</p>", user_clone));
    }
    ...
    return Html(format!("<h1>{}</h1><h3>Latest</h3><p>Posted At: {}</p><p>{}</p>",
